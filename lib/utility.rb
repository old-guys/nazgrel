...
class Utility
  class << self
    ...
    def simple_batch_operate(relation, batch_size: 50000, sleep_time: 0.2, primary_key: :id)
      records = relation.unscope(:order)

      _edge_ids = [records.klass.minimum(primary_key).to_i, records.klass.maximum(primary_key).to_i]
      _id = _edge_ids.first
      ...
      while
        ...
        _records = records.where(primary_key => _id.._id_offset)
